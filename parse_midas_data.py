...
def parse_sample_coverage_map(desired_species_name,desired_host):
    import stats_utils
    
    # Load genomic coverage distributions
    sample_coverage_histograms, samples = parse_coverage_distribution(desired_species_name,desired_host)
...
def parse_marker_gene_coverage_distribution(desired_species_name,desired_host):
    
    coverage_distribution_file = bz2.BZ2File("%s%s/snps/%s/marker_coverage_distribution.txt.bz2" % (data_directory, desired_host
, desired_species_name))
...
#
###############################################################################
def parse_snps(species_name, host, debug=False, allowed_samples=[], allowed_genes=[], allowed_variant_types=['1D','2D','3D','4D'], initial_line_number=0, chunk_size=1000000000,snps_directory=default_snps_directory):
...
def parse_subject_sample_time_map(filename=("%s/HMP1-2_ids_order.txt" % config.metadata_directory)):
