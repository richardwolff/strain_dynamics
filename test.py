...
def calculate_secondary_distance_matrix(target_avg_fs, target_avg_Ds, cluster_As, cluster_Ds):
    ...
    for i in range(0,target_avg_fs.shape[0]):
        ...
        mses = np.square(target_fs[None,:]-cluster_fs)*good_idxs
        mse_primes = np.square(1-target_fs[None,:]-cluster_fs)*good_idxs
    ...
    distance_matrix_1 = np.array(distance_matrix_1)
    distance_matrix_2 = np.array(distance_matrix_2)
    distance_matrix = np.fmin(distance_matrix_1,distance_matrix_2)
...
def calculate_distance_matrix(cluster_As, cluster_Ds):
    ...
    for i in range(0,cluster_fs.shape[0]):
        ...
        if i % 100 == 0:
            print(np.array(distance_matrix_1).shape)
...
def cluster_snps(cluster_As, cluster_Ds, max_num_snps_to_cluster=2000):
    ...
    if cluster_As.shape[0]>max_num_snps_to_cluster or cluster_As.shape[0]<2.5:
        ...
        nodes = np.ones(cluster_fs.shape[0])
    else:
        ...
        num_clusterss = np.arange(2,max_num_clusters+1)
        silhouette_scores = []
        ...
        if Smax < 0:
            nodes = np.ones(distance_matrix.shape[0])
        else:
            nodes = fcluster(Z, num_clusters, criterion="maxclust")
    ...
    for cluster_label in cluster_snp_map.keys():
        ...
        cluster_As_map[cluster_label] = np.array(cluster_As_map[cluster_label])
        cluster_Ds_map[cluster_label] = np.array(cluster_Ds_map[cluster_label])
...
def cluster_snps_by_distance(cluster_As, cluster_Ds, max_num_snps_to_cluster=2000, max_d=config.cluster_distance_threshold_reads,cluster_label_offset=0,min_coverage=5):
    ...
    Y = squareform(distance_matrix.T)
    sys.stderr.write("Done!\n")
...
def cluster_secondary_snps_by_distance(cluster_map, cluster_As, cluster_Ds, max_num_snps_to_cluster=2000, max_d=config.cluster_distance_threshold_reads,min_coverage=5):
    ...
    cluster_avg_fs = np.array(cluster_avg_fs)
    cluster_avg_Ds = np.array(cluster_avg_Ds)
    ...
    leftover_cluster_As = np.array(leftover_cluster_As)
    leftover_cluster_Ds = np.array(leftover_cluster_Ds)
...
def fast_cluster_snps_by_distance(cluster_As, cluster_Ds, max_num_snps_to_cluster=1000, max_d=config.cluster_distance_threshold_reads,min_coverage=5):
    ...
    seed_cluster_As = np.array(seed_cluster_As)
    seed_cluster_Ds = np.array(seed_cluster_Ds)
    rest_cluster_As = np.array(rest_cluster_As)
    rest_cluster_Ds = np.array(rest_cluster_Ds)
...
As = pd.read_pickle("test_A.p").astype(int).values
Ds = pd.read_pickle("test_D.p").astype(int).values

cluster_map = fast_cluster_snps_by_distance(As, Ds,max_d=300)
